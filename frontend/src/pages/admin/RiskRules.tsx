...
import { Scale, Plus, Trash2, Edit2, CheckCircle2, AlertTriangle } from 'lucide-react';
    ...
    <div className="space-y-6 pb-16">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-surface-container-lowest p-6 rounded-2xl border border-outline-variant shadow-xs">
        <div>
          ...
          <h1 className="text-xl font-bold text-primary">Deterministic Risk Rules Engine</h1>
      ...
      <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant shadow-xs overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-xs font-semibold text-on-surface-variant">Loading rules...</div>
        ) : rules.length === 0 ? (
          <div className="p-12 text-center text-xs text-on-surface-variant">No rules configured.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-outline-variant/30">
                {rules.map((rule) => (
                  <tr key={rule.id} className="hover:bg-surface-container-low">
                    <td className="py-4 px-6 font-bold text-secondary">{rule.category_code}</td>
                    <td className="py-4 px-4 font-bold text-primary max-w-[200px] truncate">
                      {rule.factor_name}
                      <p className="text-[10px] text-on-surface-variant font-normal">{rule.description}</p>
                    </td>
                    <td className="py-4 px-3 font-mono font-bold text-on-surface">{rule.condition_operator}</td>
                    <td className="py-4 px-3 font-semibold text-primary">{rule.threshold_value}</td>
                    <td className="py-4 px-3 text-center font-bold text-on-surface">{rule.likelihood_score} / 5</td>
                    <td className="py-4 px-3 text-center font-bold text-on-surface">{rule.impact_score} / 5</td>
                    <td className="py-4 px-3">
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-surface-container text-on-surface">
                        {rule.severity}
                      </span>
                    </td>
                    <td className="py-4 px-3">
                      <button
                        ...
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-full border cursor-pointer ${
                          rule.is_active
                            ? 'bg-tertiary-container/20 text-on-tertiary-container border-tertiary-fixed-dim/30'
