...
export default function OrganizationProfile() {
  const { user, updateCurrentUser, isSystemAdmin } = useAuth();
    ...
    try {
      ...
      if (!isSystemAdmin) {
        ...
        if (orgs.length > 0) {
          ...
          setBusinessType(currentOrg.business_type || 'Microfinance & Digital Lending');
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUserSuccessMsg('User information saved successfully. Your credentials and name have been refreshed.');
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-16">
          ...
          <div className="flex items-center gap-3">
            <div
              className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${
                isSystemAdmin ? 'bg-primary/10 text-primary' : 'bg-secondary/15 text-secondary'
              }`}
            >
              {isSystemAdmin ? <User className="w-6 h-6" /> : <Building2 className="w-6 h-6" />}
            </div>
            <div>
              <h1 className="text-xl sm:text-2xl font-black text-primary tracking-tight">
                {isSystemAdmin ? 'Administrator Account Profile' : 'Organization Profile & User Settings'}
              </h1>
              <p className="text-xs sm:text-sm text-on-surface-variant font-medium mt-0.5">
                {isSystemAdmin
                  ? 'Manage your administrator identity, contact details, and account credentials.'
                  : 'Manage your staff credentials and enterprise business parameters for AI risk extraction.'}
              </p>
            </div>
          </div>
      ...
      {/* 1. USER INFORMATION SECTION (Available for both Admin and Staff) */}
      <div className="space-y-4">
        <div className="flex items-center gap-2.5 px-1">
          ...
          <div>
            <h2 className="text-lg font-black text-primary tracking-tight">User Information</h2>
            <p className="text-xs text-on-surface-variant">
              {isSystemAdmin
                ? 'Personal details, email, and phone number for system governance and audit tracking.'
                : 'Personal identity and contact details displayed on assessments and audit logs.'}
            </p>
          </div>
        </div>
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <User className={`w-4 h-4 absolute left-3 top-3 ${nameError ? 'text-red-500' : 'text-on-surface-variant'}`} />
              </div>
              {nameError && (
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <Mail className={`w-4 h-4 absolute left-3 top-3 ${emailError ? 'text-red-500' : 'text-on-surface-variant'}`} />
              </div>
              {emailError && (
                <div className="mt-1.5 p-2 rounded-lg bg-red-500/10 border border-red-500/30 flex items-start gap-1.5 text-xs font-bold text-red-600 animate-in fade-in">
                  <AlertCircle className="w-3.5 h-3.5 shrink-0 mt-0.5 text-red-600" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <Phone className={`w-4 h-4 absolute left-3 top-3 ${phoneError ? 'text-red-500' : 'text-on-surface-variant'}`} />
              </div>
              {phoneError && (
                <div className="mt-1.5 p-2 rounded-lg bg-red-500/10 border border-red-500/30 flex items-start gap-1.5 text-xs font-bold text-red-600 animate-in fade-in">
                  <AlertCircle className="w-3.5 h-3.5 shrink-0 mt-0.5 text-red-600" />
          ...
          <div className="flex justify-end pt-3 border-t border-outline-variant">
            ...
            >
              ...
              <span>{savingUser ? 'Updating User Info...' : 'Save User Information'}</span>
            </button>
          </div>
        </form>
      </div>

      {/* 2. ENTERPRISE ORGANIZATION PROFILE SECTION (Hidden for System Admin, Visible for Risk Officers) */}
      {!isSystemAdmin && (
        <div className="space-y-4">
          <div className="flex items-center gap-2.5 px-1">
            ...
            <div>
              <h2 className="text-lg font-black text-primary tracking-tight">Enterprise Organization Profile</h2>
              <p className="text-xs text-on-surface-variant">
          ...
          >
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="block text-xs font-black uppercase tracking-wider text-primary">
                  Organization Legal / Trade Name <span className="text-secondary">*</span>
                </label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <Briefcase className={`w-4 h-4 absolute left-3 top-3 ${businessTypeError ? 'text-red-500' : 'text-on-surface-variant'}`} />
                </div>
                {businessTypeError && (
                  <div className="mt-1.5 p-2 rounded-lg bg-red-500/10 border border-red-500/30 flex items-start gap-1.5 text-xs font-bold text-red-600 animate-in fade-in">
                    <AlertCircle className="w-3.5 h-3.5 shrink-0 mt-0.5 text-red-600" />
            ...
            <div className="p-4 rounded-xl bg-surface-container-low border border-outline-variant/60 space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  />
                  {districtError && (
                    <p className="text-[10px] text-red-500 font-bold mt-1">{districtError}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  />
                  {sectorError && (
                    <p className="text-[10px] text-red-500 font-bold mt-1">{sectorError}</p>
                  )}
                </div>
                ...
                <div>
                  <label className="block text-[11px] font-bold text-on-surface mb-1">
                    Street Number <span className="text-on-surface-variant font-normal">(Optional)</span>
                  </label>
                  <input
                    type="text"
                    value={streetNumber}
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                {contactEmailError && <span className="text-[10px] font-bold text-red-500">Invalid</span>}
              </div>
              <div className="relative">
                <input
                  type="email"
                ...
                />
                <Mail className={`w-4 h-4 absolute left-3 top-3 ${contactEmailError ? 'text-red-500' : 'text-on-surface-variant'}`} />
              </div>
              {contactEmailError && (
                <div className="mt-1.5 p-2 rounded-lg bg-red-500/10 border border-red-500/30 flex items-start gap-1.5 text-xs font-bold text-red-600 animate-in fade-in">
                  <AlertCircle className="w-3.5 h-3.5 shrink-0 mt-0.5 text-red-600" />
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="block text-xs font-black uppercase tracking-wider text-primary">
                  Operational Scope & Business Description <span className="text-secondary">*</span>
                </label>
            ...
            <div className="flex justify-end pt-3 border-t border-outline-variant">
              ...
              >
                <Save className="w-4 h-4" />
                <span>{savingOrg ? 'Updating Profile...' : 'Save Organization Profile'}</span>
              </button>
            </div>
          </form>
        </div>
